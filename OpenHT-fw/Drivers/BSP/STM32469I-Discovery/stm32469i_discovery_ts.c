...
uint8_t BSP_TS_Init(uint16_t ts_SizeX, uint16_t ts_SizeY)
{
  ...
  if((ts_id1 == FT6206_ID_VALUE) || (ts_id2 == FT6206_ID_VALUE) || (ts_id2 == FT6X36_ID2_VALUE))
  {
    ...
    {
      ts_orientation = TS_PORT_XY; //TS_SWAP_XY | TS_SWAP_X | TS_SWAP_Y; //TS_SWAP_NONE;
    }
    else
    {
...
uint8_t BSP_TS_GetState(TS_StateTypeDef *TS_State)
{
  ...
  if(TS_State->touchDetected)
  {
    for(index=0; index < TS_State->touchDetected; index++)
    {
      ...
      if(ts_orientation & TS_PORT_XY)
      {
    	Raw_x[index] = FT_6206_MAX_HEIGHT - 1 - Raw_x[index];
        Raw_y[index] = FT_6206_MAX_WIDTH - 1 - Raw_y[index];
      }
